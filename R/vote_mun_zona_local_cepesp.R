...
vote_mun_zona_local_cepesp <- function(position, year){
  
  test_local_possition(position)
  test_local_year(year)
  
  message("Processing the data...")
  dados <-  GET(paste0("http://cepesp.io/api/consulta/votos?ano=", year, "&cargo=",
                       replace_position_cod(position), "&agregacao_regional=7"))
