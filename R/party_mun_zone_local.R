...
#'
#' @return \code{party_mun_zone_local()} returns a \code{data.frame}.
#'
#' @import utils
#' @importFrom magrittr "%>%"
#' @export
...
party_mun_zone_local <- function(year, 
                                 uf = "all",
                                 encoding = "latin1", 
                                 temp = TRUE){


  .Deprecated("elections_tse", msg = "party_mun_zone_local is deprecated and will no longer be supported in electionsBR 1.0.0. Please use the elections_tse function")
  
  answer <- party_mun_zone(year, 
                           uf,
                           encoding, 
                           temp)
  return(answer)
